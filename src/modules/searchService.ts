...
export async function search(query: string, filters: SearchFilters = {}): Promise<SearchResult[]> {
  ...
  const hasMetaFilter = (tags && tags.length > 0) || yearFrom != null || yearTo != null || !!itemType;
  ...
  if (hasMetaFilter) {
    const baseIds = candidateItemIds ?? await embeddingStorage.getIndexedItemIds();
    const filtered: number[] = [];

    for (const id of baseIds) {
      const item = Zotero.Items.get(id) as Zotero.Item | false;
      const target = item && (item as any).isAttachment?.() && (item as any).parentItem
        ? (item as any).parentItem as Zotero.Item
        : item || false;
      if (!target) continue;

      if (tags && tags.length > 0) {
        const itemTags = ((target as any).getTags() as { tag: string; type: number }[])
          .filter((t) => t.type === 0)
          .map((t) => t.tag);
  ...
  return top.map((result) => {
    ...
    const parent = item && (item as any).parentItem ? (item as any).parentItem : item || false;
    ...
    const authors = parent
      ? (() => {
          const creators = (parent as any).getCreators() as { firstName?: string; lastName?: string }[];
    ...
    const rawDate = parent ? ((parent as any).getField("date") as string | undefined) : undefined;
