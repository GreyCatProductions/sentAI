...
describe("PdfIndexer", function () {
    ...
    it("should replace chunks when re-indexed with changed content", async function () {
        ...
        const originalText = "This is the original paragraph about machine learning and how neural networks can be used to classify images and predict outcomes in complex datasets.";
        const changedText = "This is a completely different paragraph about quantum physics and how entanglement enables faster-than-classical information processing in certain computational models.";
        ...
        try {
            (Zotero.PDFWorker as any).getFullText = async () => ({ text: originalText });
            await PdfIndexer.process(fakeItem);
            const firstSave = await embeddingStorage.load(fakeItemId);

            (Zotero.PDFWorker as any).getFullText = async () => ({ text: changedText });
            await PdfIndexer.process(fakeItem);
    ...
    it("should produce identical records when re-indexed with unchanged content", async function () {
        ...
        const text = "This is a stable paragraph about neural networks and deep learning architectures that remain consistent across multiple indexing runs without any modification.";
