...
describe("search — collection filter", function () {
  ...
  before(async function () {
    ...
    const mlText = Array(5)
      .fill(
        "Machine learning and neural networks are used for pattern recognition and prediction tasks in data science.",
      )
      .join("\n\n");
    const qpText = Array(5)
      .fill(
        "Quantum physics describes wave-particle duality and superposition states in subatomic systems.",
      )
      .join("\n\n");

    await indexFakeText(
      makeFakeItem(ITEM_ML_ID, "SCOL001", "ML Paper"),
      mlText,
    );
    await indexFakeText(
      makeFakeItem(ITEM_QP_ID, "SCOL002", "QP Paper"),
      qpText,
    );
    ...
    (Zotero.Collections as any).get = (id: number) => {
      if (id === COL_ML_ID)
        return { getChildItems: () => [{ id: ITEM_ML_ID }] };
      if (id === COL_QP_ID)
        return { getChildItems: () => [{ id: ITEM_QP_ID }] };
  ...
  it("with collectionId only returns results from that collection's items", async function () {
    this.timeout(15000);
    const results = await search("machine learning quantum physics", {
      collectionId: COL_ML_ID,
    });
    assert.isArray(results);
    for (const r of results) {
      assert.strictEqual(
        r.title,
        "ML Paper",
        `Got result from outside collection: "${r.title}"`,
      );
    }
  });
